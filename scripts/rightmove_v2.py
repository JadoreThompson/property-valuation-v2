...
async def scrape_economic_relations(row):
    ...
    for key, df, id in keys:
        try:
            row[key] = df[(df["year"] == int(row["year"])) & (df["month"] == month)][id].values[0]
        except IndexError:
            row[key] = pd.NA
    ...
    for key in m_keys:
        try:
            row["_".join(key.split())] = \
                mortgage_rate[(mortgage_rate["year"] == int(row["year"])) &
    ...
    if int(row["year"]) < 2024:
        try:
            value = \
            regional_employment[(regional_employment["Year"] == int(row["year"])) & (regional_employment["Area name"] == row["district"])]["Value"].values
    ...
    if int(row["year"]) < 2023:
        try:
            value = regional_gdp[regional_gdp['borough'] == row["district"]]['2022'].values
            result = [value[0] if len(value) > 0 else pd.NA]
            row[r_keys[1]] = result
        except IndexError:
            row[r_keys[1]] = pd.NA
...
async def scrape_ammenities(row):
    print("Entered scrape ammenities...")
    lat, lng = await fetcher.get_lat_long(row["postcode"])
    row["lat"] = lat
    row["lng"] = lng
...
async def scrape_more_features_from_face(row):
    print("Entered scrape more face features...")
    more_features = await fetcher.get_epc_rating(row["address"], row["postcode"])

    row["epc_rating"] = more_features[0]
    row["sqm"] = more_features[1]
    row["council_tax_band"] = await fetcher.get_council_tax_band(row["address"], row["postcode"])
    row["crime_rate"] = await fetcher.get_crime_rate(row["address"], row["postcode"])
    return row


async def scrape_face(page, row):
    print("Entered scrape face...")
    feature_locators = [
...
def find_most_similar(full_address, page_listings):
    most_similar = max([(item, fuzz.partial_ratio(full_address[0], item)) for item in page_listings])
    return most_similar
...
async def scrape_postcode(page, row):
    ...
    try:
        ...
        await input_locator.fill(row["postcode"])
    ...
    try:
        ...
        most_similar = find_most_similar(row["address"], page_listings)
        await page.locator(f"a:has-text('{most_similar[0]}')").click(timeout=5000)
        row = await scrape_face(page, row)
        row = await scrape_more_features_from_face(row)
        row = await scrape_ammenities(row)
